...
public class SysStateController {
    ...
    public SysStateController(SysSellerStateService sellerStateService) {
     ...
     */
    @PostMapping(value="/stateFromLogin",consumes={"application/json"})
    public ResponseEntity<Object> stateFromLogin(@RequestBody @Valid Seller seller, BindingResult bindingResult)
        ...
        try {
            ...
            Map state = sellerStateService.generateStateBySeller(seller);
            System.out.println(state);

            return ResponseHandler.generateResponse("State generate SUCCESS", HttpStatus.OK, state);
        } catch(Exception e) {
            e.printStackTrace();
            return ResponseHandler.generateResponse("State generate Failed", HttpStatus.MULTI_STATUS, e.getMessage());
        }
    }
